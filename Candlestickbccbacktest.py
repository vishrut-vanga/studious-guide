...
# Example functions for calculating indicators
...
df = pd.read_csv('https://raw.githubusercontent.com/MainakRepositor/Datasets/master/Cryptocurrency/bitcoin.csv')  # Replace with your data source
df['Date'] = pd.to_datetime(df['Date'])
df.set_index('Date', inplace=True)

# Reverse the rows
df_rev_Rows = df.iloc[::-1].copy()

# Calculate indicators
df_rev_Rows = calculate_bollinger_bands(df_rev_Rows)
df_rev_Rows = calculateRSI(df_rev_Rows)
df_rev_Rows = calculate_sma(df_rev_Rows)
df_rev_Rows = calculate_macd(df_rev_Rows)
df_rev_Rows = calculate_ema(df_rev_Rows, window=50)   # 50-period EMA
df_rev_Rows = calculate_atr(df_rev_Rows)
df_rev_Rows = calculate_stochastic_oscillator(df_rev_Rows)

# Verify columns
expected_columns = ['RSI', 'Bollinger_High', 'Bollinger_Low', 'EMA_50', 'ATR', '%K', '%D']
for col in expected_columns:
    if col not in df_rev_Rows.columns:
        raise ValueError(f"Column '{col}' is missing from the DataFrame.")

# Drop rows with NaN values created by rolling windows
df_rev_Rows.dropna(subset=expected_columns, inplace=True)
...
def mean_reversion_strategy(data, oversold_threshold=30, overbought_threshold=70):
    ...
    signals['Buy_Signal'] = np.where(
        (((signals['RSI'] < oversold_threshold) | (signals['Bollinger_High'] > 70.0)) |
        (signals['%K'] < oversold_threshold)) |
        (signals['Price'] < signals['EMA_50']) | 
        (signals['ATR'] < signals['ATR'].rolling(14).mean()), 1, 0)
    
    signals['Sell_Signal'] = np.where(
        (signals['RSI'] > overbought_threshold) | 
        (signals['%K'] > overbought_threshold) |
        (signals['Price'] > signals['EMA_50']) | 
        (signals['ATR'] > signals['ATR'].rolling(14).mean()) | (signals['Bollinger_Low'] < 20.0), -1, 0)
...
def backtest_strategy(data, signals, initial_balance=1000):
...
# Example usage
trade_log, final_balance = backtest_strategy(df_rev_Rows, signals, initial_balance=1000)
print(f'Final Balance: {final_balance:.2f}')
for trade in trade_log:
    print(trade)

def evaluate_performance(trade_log, initial_balance=1000):
